...
WEB_GEOJSON_OUTPUT = WEB_DATA_DIR / "shanghai_15mc_h3_track_b.geojson"
WEB_COMPACT_OUTPUT = WEB_DATA_DIR / "hexes.geojson"
...
def load_poi_context() -> tuple[dict[str, Counter], dict[str, Counter], dict[str, Counter]]:
    ...
    with POI_INPUT.open("r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (
                ...
                and any(
                indicator in row["indicators"].split("|")
    ...
    return by_h3_indicator, by_h3_names, by_h3_district


def aggregate_rows() -> list[dict[str, object]]:
    poi_counts, poi_names, poi_districts = load_poi_context()
...
def write_geojson(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    compact_geojson = {
        "type": "FeatureCollection",
        "name": "Shanghai 15MC Track B compact web data",
        "features": compact_features,
    }
    GEOJSON_OUTPUT.write_text(json.dumps(geojson, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    WEB_DATA_DIR.mkdir(parents=True, exist_ok=True)
    WEB_GEOJSON_OUTPUT.write_text(json.dumps(geojson, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    WEB_COMPACT_OUTPUT.write_text(json.dumps(compact_geojson, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    return geojson
...
def main() -> None:
    ...
    print(f"Wrote {WEB_GEOJSON_OUTPUT}")
    print(f"Wrote {WEB_COMPACT_OUTPUT}")
